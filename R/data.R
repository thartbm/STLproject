  ...
  for (source in get) {
    for (group in groups) {
      if (source =='demographics') {
        filelist[['data']] <- c(filelist[['data']], sprintf('demographics_%s.csv',group))
      } else {
        filelist[['data']] <- c(filelist[['data']], sprintf('STL_%s_%s.zip',group,source))
      }
    }
  }
  
  
  Reach::downloadOSFdata(repository='qy5a6',
                         filelist=filelist, 
                         folder='data', 
                         unzip=TRUE, 
                         removezips=TRUE)
...
loadPerformanceFile <- function(group, id) {
  
  # Read file for participant id
  if (file.exists(sprintf('data/STL_%s/short/%s_performance.csv', group, id))) {
    filename <- sprintf('data/STL_%s/short/%s_performance.csv', group, id) 
  } else if (file.exists(sprintf('data/STL_%s/full/%s_performance.csv', group, id))) {
    filename <- sprintf('data/STL_%s/full/%s_performance.csv', group, id) 
  } else {
    cat(sprintf('No data found for participant %s in group %s\n', id, group))
  }
...
Preprocess_file <- function(group, id, condition) {
...
getidentifiers <- function(group) {
  ...
  if (file.exists(sprintf('data/demographics_%s.csv', group))) {
    demo <- read.csv(sprintf('data/demographics_%s.csv', group), stringsAsFactors = FALSE)
    if (any(c(dir.exists(sprintf('data/STL_%s/short/',group)),
              dir.exists(sprintf('data/STL_%s/full/',group))))) {
      return(demo$participant)
    } else {
      cat('WARNING: participant list returned; but there may be no data\n')
      return(demo$participant)
    }
  }
  if (dir.exists(sprintf('data/STL_%s/short/',group))) {
    csv_files <- list.files(sprintf('data/STL_%s/short/',group), pattern = "_performance.csv", full.names = TRUE)
    identifiers_list <- c()
    for (file in csv_files) {
      identifier <- gsub("_performance.csv", "", basename(file))
      identifiers_list <- c(identifiers_list, identifier)
...
Preprocess_all_files <- function() {
  ...
  for (group in c(45,60,90)) {
    ...
    identifiers <- getidentifiers(as.character(group))
    ...
    for (condition in triallabels) {
      ...
      for (identifier in identifiers) {
        processed_data <- Preprocess_file(group, identifier, condition)
        processed_data$target <- list('stl-target-rotation'='point', 'stl-arc-rotation'='arc')[[condition]]
        if (max(processed_data$rotation) != group) {
          next
        }
      ...
      write.csv( x= condition_data,
                 file= sprintf('data/%s_%d.csv', filemap[[condition]], group),
                 quote=TRUE,
                 row.names =FALSE) 
    }
  }
