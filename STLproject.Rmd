...
To summarize our findings, implicit single trial learning rates are:

1. sensitive to task errors
2. decreased with terminal feedback, but 
3. do not further decrease with delayed feedback
4. not attenuated for larger errors, but
5. capped at a maximum of around 6°
6. somewhat predictive of prolonged adaptation
7. stable across time for the cap parameter of our model
...
First, the aftereffects look really small. Are there actually any reach aftereffects after only 1 trial with terminal feedback? We do a t-test versus 0 for the average reach aftereffects across rotations in the 0 ms delay condition:
...
Is that difference with the 45 degree group significant though? It sure looks like it in the figure, but here we test this anyway with a 2x6 ANOVA, using group (45 degree group vs. the 0 ms delay / terminal feedback condition) and the 6 shared rotations sizes as factors.

```{r}
terminalFeedbackANOVA()
```
There is a main effect of group (F(1,79)=80.60, p<.001), and no other effects. This means that terminal feedback decreases initial implicit adaptation compared to continuous feedback.

Let's test any effects of delays with a 5x6 repeated measures ANOVA:

```{r}
delayedFeedbackANOVA()
```

So we can see that terminal feedback decreases initial implicit adaptation but does not eliminate it, and that further delays have no effect on initial implicit adaptation. It also doesn't really matter which rotation size is used.

Of course, we test 6 (absolute) rotation sizes and 5 delays, so we do not have 10 repetitions per condition as in the main experiment here, but "only" 8, and 240 STL bouts per participant. Nevertheless, with a sample size of 50 participants, we can be fairly confident that there is no effect of feedback delay on initial implicit adaptation. Studies reporting that delayed feedback eliminates implicit adaptation, probably measure the effect of terminal feedback instead, and this does _not_ eliminate implicit adaptation but rather reduces it.
